...
public class P740_Robber_Must_ToDo_Medium {
    /*
    ...
    public int deleteAndEarn(int[] nums) {
        ...
        dp[1] = costs[1];
        for(int i=2; i<=n; i++) {
            dp[i] = Math.max(dp[i-1], dp[i-2] + costs[i]*i);
        }
        return dp[n];
    }
    ...
    public int deleteAndEarn(int[] nums) {
        ...
        for(int value: values) {
            int temp = prev1;
            prev1 = Math.max(prev1, prev2 + value);
            prev2 = temp;
        }
        return prev1;
    }
    ...
    public int deleteAndEarn(int[] nums) {
        ...
        int step1 = 0, step2= 0;
        for(int i=max; i>=0; i--) {
            int maxValue = Math.max(step1, step2+dist[i]);
            step2 = step1;
            step1 = maxValue;
        }
        return step1;
