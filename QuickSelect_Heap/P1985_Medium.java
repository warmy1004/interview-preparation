...
public class P1985_Medium {
    ...
    String quickSelect(String[] nums, int start, int end, int k){
    ...
    int partition(String[] nums, int start, int end) {
        int pivot = start;
        for(int i=start; i<end; i++) {
            if(compareStrings(nums[i], nums[end]) <= 0 ) {
                swap(nums, pivot, i);
                pivot++;
            }
        }
