...
public class P502_ToDo_Hard {
    ...
    public int findMaximizedCapital(int k, int w, int[] profits, int[] capital) {
        ...
        // PriorityQueue is a min heap as default, but we need a max heap. So, we use Collections.reverseOrder().
        PriorityQueue<Integer> queue = new PriorityQueue<>(n, Collections.reverseOrder());
        int i=0;
        for(int j=0; j<k; j++) {
            while(i<n && projects[i].capital <= w) {
                queue.add(projects[i].profit);
                i++;
            }
